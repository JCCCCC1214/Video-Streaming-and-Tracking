                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], det[:, 4])):
                        ...
                        if save_vid or save_crop or show_vid:  # Add bbox to image
                            ...
                            if(Xl>=0 and Yl>=0):                               
                                if(Xl>=bbox_left and  Xl<=bbox_left+bbox_w and Yl>=bbox_top and  Yl<=bbox_top+bbox_h):
                                    if(flags[id]==0):
                                        flags[id]=1
                                    Xl=-1
                                    Yl=-1
                            elif(Xr>=0 and Yr>=0):
                                if(Xr>=bbox_left and  Xr<=bbox_left+bbox_w and Yr>=bbox_top and  Yr<=bbox_top+bbox_h):
                                    if(flags[id]==1):
                                        flags[id]=0
                                    Xr=-1
                                    Yr=-1
